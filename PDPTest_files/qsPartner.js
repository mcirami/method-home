...
QuidsiMaskPopup.prototype.build = function () {
    //IE6SelectHack
    if (QuidsiHelper.browser.msie && parseInt(QuidsiHelper.browser.version) <= 6) {
...
var quidsiPartner = {
    ...
    addToCart: function (sku, qty, fn, timeOut, timeOutFn) {
        ...
        var skuId = sku || quidsiPartnerInfo.bindingSku;

        var addToCartUrl = "http://" + quidsiPartnerInfo.quidsiSite + "/Services/PublishingPartnerService.aspx?method=AddToCart&partnerCode=" + quidsiPartnerInfo.partnerCode + "&sku=" + skuId + "&quantity=" + qty + "&callback=?";
        jQuery.getJSON(addToCartUrl, function (data) {
            if (data && data.itemList) {
                ...
                fn();
                setTimeout("quidsiPartner.closeMiniCart()", 5000);
            }
        });
    },
    ...
    checkout: function () {
        var quidsiCheckoutUrl = "http://" + quidsiPartnerInfo.quidsiSite + "/PublishingPartners/PartnerCheckOut.aspx?partnerCode=" + quidsiPartnerInfo.partnerCode;
...
var quidsiUI = {
    ...
    showCartInfo: function () {
        ...
        for (var i = 0; i < quidsiPartner.cartItemList.length; i++) {
            ...
            cartItemsHtml += '                      <tr>';
            cartItemsHtml += '							<td><img alt="' + item.brandName + ' ' + item.productShortName + '" src="' + item.imageUrl + '" /></td>';
            cartItemsHtml += '							<td class="jcart-item-name"><a href="' + item.partnerPDPUrl + '">' + item.brandName + ' ' + item.productShortName + '</a></td>';
            cartItemsHtml += '							<td class="jcart-item-qty"><input id="jcart-item-id-' + item.basketItemId + '" type="text" name="jcart_item_qty[ ]" size="2" value="' + item.qty + '" /></td>';
            cartItemsHtml += '							<td class="jcart-item-price"><span>' + priceString + '</span></td>';
            cartItemsHtml += '							<td class="jcart-item-delete"><a class="jcart-remove" href="javascript:void(0);" onclick="quidsiUI.handleRemoveItem(' + item.basketItemId + ');"><img alt="remove" src="http://soap.suryani.cn/Images/PublishingPartners/Method/btn-remove-item.png" /></a></td>';
            cartItemsHtml += '                      </tr>';
        }
        ...
        cartHtml += '		        <a href="javascript:void(0);" class="view-cart-link cart-state" onclick="quidsiUI.partnerCartInit()">view cart (' + quidsiPartner.cartItemList.length + ')</a>';
        cartHtml += '		    </div>';
        cartHtml += '		    <div id="jcartCart" style="display: none">';
        cartHtml += '			    <form id="jcart-inner">';
        cartHtml += '			        <fieldset>';
        cartHtml += '		                <p class="header">';
        cartHtml += '		                    <span class="header-text">' + quidsiPartner.cartItemList.length + '&nbsp;items in your cart</span>';
        cartHtml += '		                    <a href="#" class="jcart-close-button cart-state" onclick="return false;"><img src="PDPTest_files/btn-close-cart.png" alt="Close" style="width: 17px;height: 16px;"></a>';
        cartHtml += '		                </p>';
        cartHtml += '			            <table>';
        cartHtml += '				            <tbody>';
        cartHtml += '				                <tr>';
        ...
        cartHtml += '			                <input id="jcart-checkout" name="jcart-checkout" class="jcart-button form-submit" type="buttom" value="check out now with soap.com ›" onclick="quidsiUI.showTransitionHopup()" />';
        ...
        //jQuery('#global_header_mobile #QuidsiCart').html(cartHtml);
    ...
    showTransitionHopup: function () {
        ...
        var hopupHtml = '';
        hopupHtml += '<div id="soapTransitionPanel">';
        ...
        quidsiUI.autoRedirectTimer = setTimeout("quidsiPartner.checkout()", 10000);
    },
    ...
    partnerCartInit: function () {
        ...
        //IE6SelectHack: use JS to set the cart position
        if (QuidsiHelper.browser.msie && parseInt(QuidsiHelper.browser.version) <= 6) {
            quidsiUI.cartInitSlide();
        }
    },
